...
(function(S, K) {
	...
	(function(a) {
		...
		a.Fx.prototype = {
			dSetter: function() {
				var a = this.paths[0],
					...
					t;
				if (1 === u) e = this.toD;
				else if (x === f.length && 1 > u) for (; x--;) t = parseFloat(a[x]), e[x] = isNaN(t) ? f[x] : u * parseFloat(f[x] - t) + t;
				else e = f;
		...
		a.numberFormat = function(n, f, e, u) {
			...
			var x = a.defaultOptions.lang,
				...
				w, y, c = n.toString().split("e"); - 1 === f ? f = Math.min(t, 20) : a.isNumber(f) ? f && c[1] && 0 > c[1] && (w = f + +c[1], 0 <= w ? (c[0] = (+c[0]).toExponential(w).split("e")[0], f = w) : (c[0] = c[0].split(".")[0] || 0, n = 20 > f ? (c[0] * Math.pow(10, c[1])).toFixed(f) : 0, c[1] = 0)) : f = 2;
	...
	(function(a) {
		...
		k(C.prototype, {
			...
			zIndexSetter: function(a, r) {
				...
				if (m) {
					...
					r = b.childNodes;
					for (h = r.length - 1; 0 <= h && !c; h--) if (d = r[h], m = d.getAttribute("data-z-index"), H = !w(m), d !== k) if (0 > a && H && !z && !h) b.insertBefore(k, r[h]), c = !0;
		...
		k(F.prototype, {
			...
			init: function(a, g, r, d, b, k) {
				...
				z = d.element;
				a.appendChild(z);
				f(a, "dir", "ltr"); - 1 === a.innerHTML.indexOf("xmlns") && f(z, "xmlns", this.SVG_NS);
				this.isSVG = !0;
				this.box = z;
				this.boxWrapper = d;
				this.alignedObjects = [];
				this.url = (l || m) && h.getElementsByTagName("base").length ? Q.location.href.split("#")[0].replace(/<[^>]*>/g, "").replace(/([\('\)])/g, "\\$1").replace(/ /g, "%20") : "";
				this.createElement("desc").add().element.appendChild(h.createTextNode("师出网校"));
	...
	var W = function(a) {
			...
			a.extend(E.prototype, {
				...
				defaultLabelFormatter: function() {
					...
					else if (h && 1E3 <= g) for (; h-- && void 0 === l;) b = Math.pow(m, h + 1), g >= b && 0 === 10 * r % b && null !== k[h] && 0 !== r && (l = a.numberFormat(r / b, -1) + k[h]);
					...
					h(this, "getSeriesExtremes", null, function() {
						...
						y(a.series, function(g) {
							if (g.visible || !r.options.chart.ignoreHiddenSeries) {
								...
								if (a.isXAxis) b = g.xData, b.length && (g = n(b), c = I(b), v(g) || g instanceof Date || (b = q(b, v), g = n(b), c = I(b)), b.length && (a.dataMin = Math.min(D(a.dataMin, b[0], g), g), a.dataMax = Math.max(D(a.dataMax, b[0], c), c)));
				...
				getMinorTickPositions: function() {
					...
					else for (r = k + (b[0] - k) % d; r <= m && r !== c[0]; r += d) c.push(r);
				...
				renderUnsquish: function() {
					...
					else if (h && (p = q, !e)) for (f = "clip", q = d.length; !l && q--;) if (B = d[q], B = c[B].label) B.styles && "ellipsis" === B.styles.textOverflow ? B.css({
						textOverflow: "clip"
	...
	(function(a) {
		...
		C.prototype.getLogTickPositions = function(a, u, x, t) {
			...
			else if (.08 <= a) for (var y = Math.floor(u), h, p, k, q, d, e = .3 < a ? [1, 2, 4] : .15 < a ? [1, 2, 4, 6, 8] : [1, 2, 3, 4, 5, 6, 7, 8, 9]; y < x + 1 && !d; y++) for (p = e.length, h = 0; h < p && !d; h++) k = this.log2lin(this.lin2log(y) * e[h]), k > u && (!t || q <= x) && void 0 !== q && c.push(q), q > x && (d = !0), q = k;
	...
	(function(a) {
		...
		a.Point.prototype = {
			...
			optionsToObject: function(c) {
				...
				else if (u(c)) for (!f && c.length > b && (h = typeof c[0], "string" === h ? e.name = c[0] : "number" === h && (e.x = c[0]), v++); n < b;) f && void 0 === c[v] || (0 < d[n].indexOf(".") ? a.Point.prototype.setNestedProperty(e, c[v], d[n]) : e[d[n]] = c[v]), v++, n++;
	...
	(function(a) {
		...
		a.Series = a.seriesType("line", null, {
			...
			updateData: function(b) {
				...
				if (m) for (b = d.length; b--;) g = d[b], g.touched || g.remove(!1), g.touched = !1;
			...
			setData: function(b, c, d, e) {
				...
				if (!N) {
					...
					if (B && f > B) {
						...
						if (k(G)) for (d = 0; d < f; d++) u[d] = this.autoIncrement(), D[d] = b[d];
						else if (p(G)) if (y) for (d = 0; d < f; d++) G = b[d], u[d] = G[0], D[d] = G.slice(1, y + 1);
						else for (d = 0; d < f; d++) G = b[d], u[d] = G[0], D[d] = G[1];
						else a.error(12)
			...
			getExtremes: function(a) {
				...
				d = a.length;
				for (B = 0; B < d; B++) if (l = c[B], v = a[B], f = (k(v, !0) || p(v)) && (!b.positiveValuesOnly || v.length || 0 < v), l = this.getExtremesFromAll || this.options.getExtremesFromAll || this.cropped || (c[B + q] || l) >= m && (c[B - q] || l) <= h, f && l) if (f = v.length) for (; f--;)"number" === typeof v[f] && (e[g++] = v[f]);
				else e[g++] = v;
				this.dataMin = n(e);
				this.dataMax = I(e)
			},
			...
			update: function(a, b, h, f) {
				...
				u("xAxis yAxis zAxis series colorAxis pane".split(" "), function(b) {
					...
					a[b] && ("series" === b && (c = [], u(l[b], function(a, b) {
						...
						if (!d && h) if ("series" === b) l.addSeries(a, !1).touched = !0;
	...
	(function(a) {
		...
		c.prototype.drawDataLabels = function() {
			...
			function c(a, b) {
				...
				else if (q(a) || q(b)) for (d = Math.max(a.length, b.length); d--;) c[d] = x(a[d], b[d]);
				else c = x(a, b);
				return c
			}
			var f = this,
		...
		x(q.prototype, {
			...
			setState: function(a, b) {
				...
				if (!(a === this.state && !b || this.selected && "select" !== a || !1 === h.enabled || a && (g || n && !1 === q.enabled) || a && v.states && v.states[a] && !1 === v.states[a].enabled)) {
					...
					if (this.graphic) this.state && this.graphic.removeClass("highcharts-point-" + this.state), a && this.graphic.addClass("highcharts-point-" + a), this.graphic.animate(f.pointAttribs(this, a), k(u.options.chart.animation, h.animation)), y && this.graphic.animate(y, k(u.options.chart.animation, q.animation, l.animation)), p && p.hide();
					else {
						if (a && q) {
							l = v.symbol || f.symbol;
		...
		C.prototype.currentOptions = function(e) {
			function t(e, c, h, p) {
				...
				a.objectEach(e, function(a, d) {
					if (!p && -1 < I(d, ["series", "xAxis", "yAxis"])) for (a = u(a), h[d] = [], k = 0; k < a.length; k++) c[d][k] && (h[d][k] = {}, t(a[k], c[d][k], h[d][k], p + 1));
