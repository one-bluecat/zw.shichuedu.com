<div class="box">
    ...
    <div class="i_jobcz">
        ...
        <ul id="nav">
            <?php
            foreach ($city as $k=>$v){
            ?><li><a target='_blank' href='/paiming/shuju/<?php echo $v['en'].($year!=$system_config['year']?'/'.$year.'.php':'');?>'><?php echo $v['zh'];?></a></li>
                <?php  }
                ?>
        </ul>
    </div>
    <div class="job-table">
        <!--职位内容开始-->
        <div class='job_title'><h1><?php echo $year;?>安徽<?php echo $city_map[$city_applyNum_list['province']]['zh'];?>教师招聘考试职位表</h1><span>(<?php echo $city_applyNum_list['post_num'];?>个职位、<?php echo $city_applyNum_list['apply_num'];?>人)</span></div>
