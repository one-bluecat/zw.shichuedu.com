...
class post extends MY_Controller
{
    ...
    public function lists()
    {
        ...
        $year && $where .=  $where ? " AND (year like '%{$year}%')":" (year like '%{$year}%')";
    ...
    public  function profession(){
        $this->load_view('post/profession');
    }

    public  function profession_lists(){
        $post_field = 'id,year,profession';
        $limit = 25;
        $order_by = 'id desc';
        $data = $this->common->admin_page_list('profession', $limit, "1=1", $order_by, $post_field);
        $this->load_view('post/profession_lists', $data);
    }
    ...
    public function add_profession(){
        if ($data = $this->input->post()) {
            ...
            $addData['year'] = $data['year'];
            $addData['profession'] = $data['profession'];
            $this->common->insert('profession', $addData);
            $this->json(array('code' => 1, 'msg' => '添加成功！', 'url' => site_url('post/profession')));
        } else {
            $this->load_view('post/add_profession');
        }
    }

    public function edit_profession($id = null){
        if ($data = $this->input->post()) {
            $find = $this->common->get_one('profession', "id ='{$data['id']}'");
            if (!$find) {
                $this->json(array('code' => 0, 'msg' => '记录不存在！'));
            }
            $update['profession'] = $data['profession'];
            $this->common->update('profession', array('id' => $data['id']), $update);
            $this->json(array('code' => 1, 'msg' => '修改成功！', 'url' => site_url('post/profession')));
        } else {
            $data = $this->common->get_one('profession', array('id' => $id));
            $this->load_view('post/edit_profession', $data, true);
        }
    }

    public function del_profession($id){
        $find = $this->common->get_one('profession', array('id' => $id));
        if (!$find) {
            $this->json(array('code' => 0, 'msg' => '记录不存在！'));
        }
        $this->common->delete('profession', array('id' => $find['id']));
        $this->json(array('code' => 1, 'msg' => '删除成功！', 'url' => site_url('post/profession')));
    }
